...
def getNotes(intensity, instrument:str = DEFAULT_INSTRUMENT, extension = DEFAULT_NOTE_EXTENSION, audio_folder = AUDIO_FOLDER, create_sound:bool=True, bpm:float=DEFAULT_BPM) -> Note_group:

    path = os.path.join(ROOT_DIR, '..', NOTES_FOLDER, instrument, audio_folder, f"*.{extension}")
    file_paths = glob.glob(path)
    noteList = [Note(path = file_path, create_sound=create_sound, bpm=bpm) for file_path in file_paths if intensity in file_path] #List comprehension
